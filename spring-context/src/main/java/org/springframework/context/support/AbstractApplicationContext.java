...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	 ...
	 * 在此工厂中，国际化消息MessageSource的bean的名称
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 准备此上下文以进行刷新。
			// 调用准备刷新的方法，获取容器的当前时间,同时给容器设置同步标识，具体方法
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 告诉子类刷新内部bean工厂。
			// 告诉子类启动refreshBeanFactory()方法，Bean定义资源文件的载入从子类的refreshBeanFactory()启动，里面有抽象方法
			...
			try {
				...
				// 允许容器的子类方法去注册postProcessor，钩子方法
				...
				// 初始化国际化配置 针对不同的地区，展示不同的语言
				initMessageSource();

				// Initialize event multicaster for this context.
				// 初始化事件发布者组件
				// 初始化ApplicationEvnetMulticaster该类作为时间发布者
				...
				// 触发初始化完成的回调方法，并发布容器刷新完成的时间给监听者
				// 初始化容器的生命周期事件处理器，（默认使用DefaultLifecycleProcessor）， 调用扩展了SmartLifecycleProcessor
				// 当Spring容器加载所有bean并完成初始化之后，会接着回电实现该接口的类中对应的方法（start（）方法）
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果某个bean依赖于一瞎几个接口的实现类，在自动装配的时候忽略它们，
		...
		// ApplicationListenerDetector的作用就是判断某个Bean是否时ApplicationListener，
		// 如果是，加入到时间监听者队列
	 ...
	 * 这允许在某些应用上问实现中注册特殊的BeanPostProcessors等。
	...
	/**
	 * 实例化并调用所有已注册的BeanFactoryPostProcessor的bean,如果已给出顺序，请按照顺序。
	 * 必须在单例实例化之前调用
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// AOP分为三个方式:编译期直入，类加载器织入和运行期织入
		// LoadTimeWeaving属于第二种，主要通过jvm进行织入
		// 先初始化LoadTimeWeaverAware Bean，以便尽早注册他们的transformers
	...
	protected void finishRefresh() {
		...
		// 如果处于激活状态，将参与到LiveBeansViewMBean中。
		LiveBeansView.registerApplicationContext(this);
	}

	/**
	 *  取消此上下文的刷新尝试，在抛出异常后重置active状态。
	 *
